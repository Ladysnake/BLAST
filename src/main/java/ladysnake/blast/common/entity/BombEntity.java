            ...
            if (!this.world.isClient()) {
                for (net.minecraft.entity.player.PlayerEntity playerEntity : this.world.getPlayers()) {
                    ...
                    if (serverPlayerEntity.squaredDistanceTo(this.getX(), this.getY(), this.getZ()) < 4096.0D) {
                        serverPlayerEntity.networkHandler.sendPacket(new ExplosionS2CPacket(this.getX(), this.getY(), this.getZ(), explosion.getPower(), explosion.getAffectedBlocks(), explosion.getAffectedPlayers().get(serverPlayerEntity)));
                    }
                }
            }
        }
